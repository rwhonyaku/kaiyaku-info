...
export const PHASE1_SERVICES: ServiceRecord[] = [
...

  {
    ...
    cancellationVsAccountDeletion: [
      ...
      "アカウント管理メニューには、アドビアカウントの削除が別項目として掲載されています。",
    ],
    confirmationNotes: [
    ...
    officialSourceSummary:
      "Adobe Accountヘルプでは、アドビアカウントの「プランを管理」から体験版またはサブスクリプションを解約する流れが説明されています。Apple、Google、Microsoftなどのサードパーティ経由で購入したサブスクリプションは、それぞれの解約手順に従う扱いです。確認メール、アカウントページでの確認、複数プランの個別解約、現在の課金期間終了までのアクセス継続、請求期間内の再アクティベート、キャンセル後のCreative CloudストレージやLightroom画像の扱いも確認できます。",
    seoTitle:
...

  {
    ...
    officialSourceSummary:
      "Zoomのサポートでは、オンライン購入、営業見積もり経由、Google Playストア、Apple App Storeの購入経路ごとにキャンセル先が分かれています。Zoomウェブポータル内の請求ポータル位置に応じて、「プランと請求」または「アカウント管理」から対象プランを管理します。キャンセル要件、キャンセル保留中の表示、確認メール、アドオンを先にキャンセルする必要、クラウドレコーディングの30日間利用も同じ記事で確認できます。",
    seoTitle:
...

  {
    ...
    officialSourceSummary:
      "Rakuten Musicの公式ページでは、Webサイト上の「ご契約内容の確認・変更」から解約やプラン確認を行う導線が示されています。楽天モバイルのお客様サポートでは、無料トライアルと有料プランで確認先が分かれ、無料トライアル終了後は契約プランで自動更新されると説明されています。料金プランページには、30日間ごとの月額プラン、360日間ごとの年額プラン、アプリ内課金で申し込めるプラン、Web申込み対象プラン、バンドルプランの注記が掲載されています。",
    seoTitle:
