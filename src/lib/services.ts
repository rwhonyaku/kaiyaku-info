...
export const PHASE1_SERVICES: ServiceRecord[] = [
...

  {
    ...
    cancellationTimingNotes: [
      ...
      "サブスクリプション条件では、初回注文後14日以内に解約する場合は全額返金されると記載されています。",
      "14日経過後に解約する場合、料金は返金されず、サービスは契約期間の終わりまで継続されます。",
    ],
    postCancellationAccess: [
    ...
    officialSourceSummary:
      "Adobe Accountヘルプでは、アドビアカウントで「プランを管理」から体験版またはサブスクリプションを解約する流れが説明されています。Apple、Google、Microsoftなどのサードパーティ経由で購入したサブスクリプションは、それぞれの解約手順に従う扱いです。確認メール、アカウントページでの確認、複数プランを個別に解約する必要、現在の課金期間終了までのアクセス継続も同じページで確認できます。",
    seoTitle:
    ...
    searchIntentNotes: [
      {
        ...
        items: [
          ...
          "プランをキャンセルする際は確認ウィンドウが表示され、キャンセルが有効になる時期などの条件が通知されます。",
        ],
      },
      {
        title: "解約できない表示・再アクティベート",
        items: [
          "キャンセルオプションがグレーアウトしている場合は、自分でサブスクリプションをキャンセルできない可能性があると案内されています。",
          "「プランをキャンセル」オプションが表示されない場合は、ウェブポータルからプランをキャンセルできない旨のメッセージが表示されます。",
          "プランをキャンセルした後、キャンセルが有効になる前であれば、キャンセルリクエストを撤回して再アクティベートできます。",
        ],
      },
    ],
    contractRouteNotes: [
...

  {
    ...
    officialSourceSummary:
      "Rakuten Musicの公式ページでは、Webサイト上の「ご契約内容の確認・変更」から解約やプラン確認を行う導線が示されています。楽天モバイルのお客様サポートでは、無料トライアルと有料プランで確認先が分かれ、無料トライアル終了後は契約プランで自動更新されると説明されています。料金プランページには、アプリ内課金、Web申込み対象プラン、バンドルプランなど契約経路ごとの注記も掲載されています。",
    seoTitle:
